...
DOWNURL="http://oauwu8iy4.bkt.clouddn.com/"
...
function updateNginx {
	if [ ! -s "nginx-1.9.0.tar.gz" ];then
		wget -c http://nginx.org/download/nginx-1.9.0.tar.gz
	fi
...
function installPHP {	
	if [ ! -e "/usr/local/php/bin/php" ];then		
		...
		if [ ! -s "php-5.5.24.tar.gz" ];then
			wget -c http://cn2.php.net/distributions/php-5.5.24.tar.gz
		fi
		tar zxvf php-5.5.24.tar.gz
		cd php-5.5.24
		./configure --prefix=/usr/local/php --with-curl --enable-mbstring  --with-openssl --with-mysql=mysqlnd --with-mysqli --enable-opcache --with-pdo-mysql --with-iconv --with-gd --enable-fpm --with-jpeg-dir --with-png-dir --enable-zip --with-freetype-dir --with-gettext --enable-gd-native-ttf --without-pdo-sqlite --without-sqlite3
		make && make install
...
function installPHPMongo {
	if [ ! -s "mongo-1.6.8.tgz" ];then
		wget -c http://pecl.php.net/get/mongo-1.6.8.tgz
	fi
...
function installPHPSSH2 {
	...
	if [ ! -s "libssh2-1.4.3.tar.gz" ];then
		wget -c http://www.libssh2.org/download/libssh2-1.4.3.tar.gz
	fi
...
do
	...
	echo -e "(1) Configure \033[1mNGINX\033[0m Service"
	echo -e "(2) Configure \033[1mPHP\033[0m Service"
	echo -e "(3) Configure \033[1mMYSQL\033[0m Service"
	echo -e "(0) Quit"
